...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!apiKey || !placeId) {
    return res.status(503).json({ error: 'Missing GOOGLE_PLACES_API_KEY or GOOGLE_PLACE_ID' });
  }
  ...
  try {
    ...
    return res.status(502).json({ error: 'Failed to fetch Google Place details' });
  }
}
